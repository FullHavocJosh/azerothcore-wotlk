...
# Date: 2025-12-03
# Version: 1.2.2 (Testing - Playerbot branch with git submodule update commands)
################################################################################
...
update_core() {
	...
	log_info "Updating git submodules (init + recursive)..."
	if ! git submodule update --init --recursive 2>&1 | tee -a "${LOG_FILE}"; then
		log_warning "git submodule update --init --recursive had issues (continuing)"
	else
		log_success "Git submodules initialized recursively"
	fi

	log_info "Updating git submodules (remote + recursive)..."
	if ! git submodule update --remote --recursive 2>&1 | tee -a "${LOG_FILE}"; then
		log_warning "git submodule update --remote --recursive had issues (continuing)"
	else
		log_success "Git submodules updated to latest remote versions"
	fi
...
main() {
	...
	log_info "Environment: TESTING (Playerbot branch)"
